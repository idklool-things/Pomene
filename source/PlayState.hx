...
import stages.SchoolStage;
import stages.MallStage;
	...
	override public function create()
	{
		...
		if (curStage == null || curStage.length < 1) {
		    switch(Paths.spaceToDash(SONG.song))
		    {
		        ...
		        case 'cocoa', 'eggnog', 'winter-horrorland':
		            curStage = 'mall';
		            if (Paths.spaceToDash(SONG.song) == 'winter-horrorland')
		                curStage += 'Evil';
		        case 'senpai', 'roses', 'thorns':
		            curStage = 'school';
		            if (Paths.spaceToDash(SONG.song) == 'thorns')
		                curStage += 'Evil';
		        default:
		            curStage = 'stage';
		    }
		...
		switch(curStage)
		{
		    case 'halloween', 'spooky':
		        stagesArray.push(new Halloween());
		    case 'limo':
		        stagesArray.push(new LimoStage());
		    case 'mall':
		        stagesArray.push(new MallStage());
		    case 'mallEvil':
		        stagesArray.push(new MallEvil());
		    case 'philly':
		        stagesArray.push(new PhillyStage());
		    case 'school':
		        stagesArray.push(new SchoolStage());
		    case 'schoolEvil':
		        stagesArray.push(new SchoolEvil());
		    case 'stage':
	...
	function pushEvents(event:EventVars) {
	  //for (event in events) {
	    switch(event.name.toLowerCase()) {
	      ...
	      case 'set property':
	          if (Reflect.getProperty(this, event.val1) != null)
	              Reflect.setProperty(this, event.val1, event.val2);
	    }
			...
			if (camCanMove && FlxG.save.data.camMovement) {
			    ...
			    for (anim in Reflect.fields(directions)) {
			        ...
			        if (chars.animation.curAnim.name.contains(anim)){
			            camFollow.x += realAnim[0];
			            camFollow.y += realAnim[1];
			        }
			    }
			}
		}
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			if (SONG.needsVoices) playerVocals.volume = 1;
